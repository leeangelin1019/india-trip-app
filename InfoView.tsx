...
export const InfoView: React.FC = () => {
  ...
  const getLinkEmoji = (title: string) => {
    ...
    if (t.includes('uber')) return '🚖';
    if (t.includes('incredible')) return '🕉️';
    if (t.includes('kerala')) return '🥥';
    return '🔗';
  };
    ...
    <div className="pb-32 pt-0 animate-in fade-in duration-700">
      ...
      <div className="bg-white border-2 border-mag-black rounded-none overflow-hidden mb-10">
        ...
        {EMERGENCY_CONTACTS[2] && (
          <a href={`tel:${EMERGENCY_CONTACTS[2].number}`} className="block p-4 active:bg-gray-50 border-b border-mag-black/10">
        ...
        {EMERGENCY_CONTACTS[3] && (
          <a href={`tel:${EMERGENCY_CONTACTS[3].number}`} className="block p-4 active:bg-gray-50">
             <div className="flex items-center justify-between">
                <div>
                    <div className="text-sm font-black text-mag-black">{EMERGENCY_CONTACTS[3].title}</div>
                </div>
                <div className="text-sm font-mono font-black text-mag-red">{EMERGENCY_CONTACTS[3].number}</div>
             </div>
          </a>
        )}
      </div>
      ...
      <div className="mb-6">
        ...
        <h3 className="text-[20px] font-serif font-black text-mag-black">實用語言</h3>
      </div>
