...
import { Tab, ShoppingItem, ExpenseRecord, ChecklistItem, DaySchedule } from './types';
...
const App: React.FC = () => {
  ...
  // 行程資料狀態 (支援編輯)
  const [itineraryData, setItineraryData] = useState<DaySchedule[]>(() => safeGetItem('dynamic_itinerary', ITINERARY));
  ...
  useEffect(() => {
    ...
    localStorage.setItem('dynamic_itinerary', JSON.stringify(itineraryData));
  }, [checkedItems, todoList, carryOnList, checkedBagList, shoppingList, itineraryData]);
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        // Coordinates for Mumbai (BOM)
        const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=19.0760&longitude=72.8777&current=temperature_2m,weather_code&timezone=Asia%2FKolkata');
      ...
      const parsedData: ExpenseRecord[] = (json.data || [])
        .map((row: any): ExpenseRecord => {
          return {
            ...
            date: row.date,
            item: row.item,
            // Map the 'payer' field from GAS JSON (Column C) to paymentMethod
            // The backend returns Col C as 'payer' based on the script logic, we map it to UI's paymentMethod
            paymentMethod: row.payer || '現金',
            amountTwd: parseCurrency(row.amountTwd),
            amountInr: parseCurrency(row.amountInr),
            note: row.note
          };
        })
  ...
  return (
    <div className="relative min-h-screen font-sans text-mag-black bg-mag-paper">
      ...
      <header className="fixed top-0 left-0 right-0 z-30 pt-safe-top bg-white/90 backdrop-blur-md">
        <div className="max-w-xl mx-auto">
          <div className="flex justify-between items-center py-5 px-6">
             <div className="text-left">
                <div className="flex items-center gap-1.5 mb-1.5">
                  <span className="bg-mag-black text-white text-[10px] px-1.5 py-0.5 font-black tracking-normal leading-none">2026</span>
                  <span className="text-[10px] font-bold tracking-[0.15em] uppercase text-mag-gold">Kerala & Goa</span>
                </div>
                <h1 className="font-noto font-medium text-[20px] leading-none">South India Odyssey</h1>
             </div>
             {weather && (
               <div className="flex items-center gap-3">
                  <div className="text-right">
                    <div className="text-lg font-mono font-black text-mag-black leading-none mb-1">{weather.temp}°C</div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase leading-none tracking-tighter">MUMBAI</div>
                  </div>
          ...
          <nav className="flex w-full bg-white/50">
          ...
          {activeTab === Tab.ITINERARY && (
            <div className="flex w-full border-t border-gray-50 bg-white px-2 overflow-x-auto no-scrollbar">
              {itineraryData.map((day, idx) => (
                <button key={idx} onClick={() => setSelectedDateIdx(idx)} className="flex-shrink-0 flex flex-col items-center justify-center py-3 min-w-[50px]">
                  <div className={`flex flex-col items-center justify-center w-11 h-11 transition-all ${selectedDateIdx === idx ? 'bg-mag-black text-white' : ''}`}>
                    <div className={`text-[9px] font-bold mb-1 leading-none ${selectedDateIdx === idx ? 'text-gray-300' : 'text-mag-gray'}`}>{day.weekday.replace('星期', '')}</div>
                    <div className={`text-[11px] font-mono font-black leading-none`}>{day.date.split('/')[1]}</div>
                  </div>
                </button>
              ))}
            </div>
